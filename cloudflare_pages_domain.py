...
def get_pages_project_domains(api_token, account_id, project_name):
    ...
    url = urljoin(API_BASE_URL, f"{account_id}/pages/projects/{project_name}/domains")
...
def delete_pages_project_domin(api_token, account_id, project_name, domain_name):
    ...
    url = urljoin(API_BASE_URL, f"{account_id}/pages/projects/{project_name}/domains/{domain_name}")
...
def run_module():
    ...
    status_code, response = get_pages_project_domains(api_token, account_id, project_name)
    if not status_code in (200, 201):
        module.fail_json(msg='Error fetching project domains', error=response)

    project_exists = find_and_compare_page_project_domain(response, domain_name)

    if state == 'present':
        if project_exists:
            status_code, response = update_pages_project_domain(api_token, account_id, project_name, domain_name)
            ...
            status_code, response = create_pages_project_domain(api_token, account_id, project_name, domain_name)
            if status_code in (200, 201):
                result['changed'] = True
        ...
        if project_exists:
            status_code, response = delete_pages_project_domin(api_token, account_id, project_name, domain_name)
            if status_code in (200, 201):
                result['changed'] = True
